...
sub _bid_to_string {
    ...
    if ( substr( $binary, 1, 2 ) eq '11' ) {
        ...
        $e = unpack( "n", pack( "B*", "00" . substr( $binary, 3, 14 ) ) ) - 6176;
    }
    else {
        # Bits: 1*sign 14*exponent 113*significand
        $coef = "" . Math::BigInt->new( "0b" . substr( $binary, 15 ) );
        $e = unpack( "n", pack( "B*", "00" . substr( $binary, 1, 14 ) ) ) - 6176;
    }
    ...
    # not exponential notation (small negative exponents)
    else {
        # maybe no exponent? (i.e. positive/negative integer)
        ...
        # otherwise, decimal goes in the middle
        substr( $coef, -abs($e), 0, "." );
        return $pos ? $coef : "-$coef";
    }
}
...
sub _string_to_bid {
    ...
    # Get binary representation of biased exponent
    my $biased_exp = unpack( "B*", pack( "n", $exp + 6176 ) );
